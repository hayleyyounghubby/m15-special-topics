...
gapminder.country <- function(country_name) {
  country_filter <- filter(gapminder, country == country_name)
  country_filter %>% 
    ggplot(aes(year, lifeExp)) + 
    geom_line() + 
    ggtitle("Full data = ")
  country_filter_mod <- lm(lifeExp ~ year, data = country_filter)
  country_filter %>%
    add_predictions(country_filter_mod) %>%
    ggplot(aes(year, pred)) + 
    geom_line() + 
    ggtitle("Linear trend + ") 
  country_filter %>% 
    add_residuals(country_filter_mod) %>% 
    ggplot(aes(year, resid)) + 
    geom_hline(yintercept = 0, colour = "white", size = 3) + 
    geom_line() + 
    ggtitle("Remaining pattern")
